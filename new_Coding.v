...
Section Model.
  ...
  (* We assume the existence of a formula which strongly represents the prime number function *)
  Variable π : form.
  Variable Hπ : binary π /\
    (forall x y, Irred x = y -> Q ⊢I π[(num y)..][(num x)..]) /\
    (forall x y, Irred x <> y -> Q ⊢I ¬ π[(num y)..][(num x)..]).
...
Section Facts.
  ...
  Lemma std_mult' x y m :
    0 < m -> x i⊗ y = inu m -> std x /\ std y.
  Proof.
    intros [k <-]%lt_nat_equiv.
...
End Facts.
  ...
  Definition div_pi n a :=  (inu n .: (fun _ => inu a)) ⊨ ∃ (π ∧ ∃ $1 ⊗ $0 == $3).


  Lemma π_repr x y rho : 
    (inu y .: inu x .: rho) ⊨ π <-> Irred x = y.
  Proof.
    destruct Hπ as (_ & H1 & H2).
    destruct (dec_eq_nat (Irred x) y) as [h|h].
    - split; auto. intros _.
      rewrite <-!switch_num.
      apply H1, soundness in h. apply h.
      intros ??. apply axioms. now constructor.
    - split; intros; try tauto. exfalso.
      apply H2, soundness in h. revert H.
      rewrite <-!switch_num. apply h.
      intros ??. apply axioms. now constructor.
  Qed.


  Lemma π_equiv n a rho :
    0 < a -> ((inu n .: inu a .: rho) ⊨ ∃ (π ∧ ∃ $1 ⊗ $0 == $3)) <-> div_num (Irred n) (inu a).
  Proof.
    intros Ha.
    split.
    - intros [d [H [k Hk]]].
      cbn in Hk. unfold div_num. exists k.
      assert (std d /\ std k) as [[nd <-] [nk <-]].
      eapply std_mult'; eauto.
      apply π_repr in H. now rewrite <-H in *.
    - intros [k Hk]. exists (inu (Irred n)). split.
      + now apply π_repr.
      + cbn. now exists k.
  Qed.
...
Section Coding.
  ...
  Lemma Coding_nat A n :
    ~ ~ exists c, c > 0 /\ forall u, (u < n -> A u <-> Mod (Irred u) c = 0) /\ (Mod (Irred u) c = 0 -> u < n).
  Proof.
    induction n.
    - apply DN. exists 1. split; [lia|]. 
      intros u. split. lia.
    ...
    - assert (~ ~ (A n \/ ~ A n)) as Dec_An by tauto.
      ...
      + exists (a * Irred n). split.
        { assert (Irred n > 1). apply irred_Irred. lia. }
        intros u.
  ...
  Lemma Coding_nat_Definite A n :
    Definite A -> exists c, c > 0 /\ forall u, (u < n -> A u <-> Mod (Irred u) c = 0) /\ (Mod (Irred u) c = 0 -> u < n).
  Proof.
    intros Def_A.
    induction n.
    - exists 1. split; [lia|]. 
  ...
  Lemma Coding_model_binary alpha : 
    binary alpha -> 
    forall n rho, rho ⊨ ∀ ¬ ¬ ∃ (zero ⧀ $0) ∧ ∀ $0 ⧀ (num n) --> alpha[up $1..] <--> ∃ (π ∧ ∃ $1 ⊗ $0 == $3).
  Proof.
  ...
  Proof.
    ...
    refine (
      ...
      in _).
    destruct Ha as [a [H' Ha]].
    exists (inu a). split.
    { cbn; change i0 with (inu 0). now apply lt_equiv. }
    intros u' Hu. cbn in Hu.
    rewrite num_subst in Hu.
    setoid_rewrite eval_num in Hu.
    apply lessthen_num in Hu. 2: apply axioms.
...
Section notStd.
  ...
  Theorem Coding_nonStd_unary alpha : 
    unary alpha -> ~ ~ exists c, forall u rho, (inu u .: c .: rho) ⊨ (alpha <--> ∃ (π ∧ ∃ $1 ⊗ $0 == $3)).
  Proof.
    intros unary_alpha.
    specialize (Coding_model_unary _ unary_alpha) as H.
    assert (forall n rho, (inu n .: rho) ⊨ ¬ ¬ ∃ (zero ⧀ $0) ∧ ∀ $0 ⧀ $2 --> alpha <--> ∃ (π ∧ ∃ $1 ⊗ $0 == $3) ) as H'.
    intros n rho.
    rewrite <-switch_num. cbn -[sat].
    specialize (H n rho).
    rewrite !num_subst in *.
    assert (π[var] = π[up (up (up (num n)..))] ) as <-.
    { eapply bounded_subst. apply Hπ.
    ...
    2 : { solve_bounds.
          1 : { solve_bounds. 
                - change (bounded_t 3 (num 0)`[↑]). rewrite num_subst.
                  assert (3 >= 0) as h by lia.
                  refine (bounded_up_t _ h). apply closed_num.
                - constructor; lia.
          }
          all: repeat solve_bounds.
          all: try eapply bounded_up; try apply binary_alpha; try apply Hπ.
          all: eauto; try lia. }
    rewrite <-NNN_N.
    ...
    + intros H. destruct Ha1 as [d Hd].
      ...
      intros []; try now intros. lia.
      exists d. split.
      eapply bound_ext. apply Hπ. 2: apply Hd.
      intros [|[]]; try now intros. lia.
      apply Hd.
    + intros [k Hk].
      ...
      intros []; try now intros. lia.
      exists k. split.
      eapply bound_ext. apply Hπ. 2: apply Hk.
      intros [|[]]; try now intros. lia.
      apply Hk.
  Qed.
  ...
  Theorem Coding_nonstd_binary_Definite alpha :
    binary alpha -> (forall b u, definite (forall rho, (inu u .: b .: rho) ⊨ alpha ) ) ->
     ~ ~ forall b, exists c, forall u rho, (inu u .: b .: c .: rho) ⊨ (alpha <--> ∃ (π ∧ ∃ $1 ⊗ $0 == $4)).
  Proof.
    intros binary_alpha Def_A.
    specialize (Coding_model_binary_Definite _ binary_alpha Def_A) as H.
    assert (forall n rho, (inu n .: rho) ⊨ ∀ ∃ (zero ⧀ $0) ∧ ∀ $0 ⧀ $3 --> alpha[up $1..] <--> ∃ (π ∧ ∃ $1 ⊗ $0 == $3) ) as H'.
    intros n rho.
    rewrite <-switch_num. cbn -[sat]. rewrite !num_subst.
    assert (π[var] = π[up (up (up (up (num n)..)))] ) as <-.
    eapply bounded_subst. apply Hπ.
    intros [|[]]; try now intros. lia.
    ...
    2 : { solve_bounds.
          1 : { solve_bounds. 
                - change (bounded_t 4 (num 0)`[↑]). rewrite num_subst.
    ...
    apply (DN_chaining H'), DN. clear H' H.
      ...
      cbn in He2. specialize (He2 (fun _ => i0) b) as [a [H' Ha]].
  ...
  Theorem Coding_nonstd alpha b : binary alpha ->
    ~ ~ exists a, forall u rho, (inu u .: b .: a .: rho) ⊨ (alpha <--> ∃ (π ∧ ∃ $1 ⊗ $0 == $4)).
  Proof.
    intros binary_alpha.
